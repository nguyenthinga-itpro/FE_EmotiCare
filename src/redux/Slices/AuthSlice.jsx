...
import Api from "../../Api/api"; // axios instance đã có withCredentials:true
...
export const loginEmail = createAsyncThunk(
  ...
  async ({ email, password }, { rejectWithValue }) => {
    try {
      ...
      // gửi token Firebase lên BE để tạo cookie HttpOnly
      const res = await Api.post("/auth/login", { idToken });
      return res.data; // BE trả về user info
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
  }
...
// === FETCH CURRENT USER ===
// Gọi khi reload để kiểm tra cookie còn hợp lệ không
export const fetchMe = createAsyncThunk(
  "auth/fetchMe",
  async (_, { rejectWithValue }) => {
    try {
      const res = await Api.get("/auth/me"); // cookie tự gửi
      console.log("✅ fetchMe response:", res.data);
      return res.data; // { user }
    } catch (err) {
      console.error("❌ fetchMe error:", err);
